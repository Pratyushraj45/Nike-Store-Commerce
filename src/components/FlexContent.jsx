...
const FlexContent = ({
  ifExist,
  endpoint: { title, heading, text, btn, img, url },
}) => {
  return (
    <>
      <div
        className={`flex item-center justify-between lg:flex-col lg:justify-center nike-container ${
      ...
      >
        <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center">
          ...
          <a
            href={url}
            className='flex items-center target={"_blank"} role="button'
          >
            <button
              type="button"
        ...
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
          <img
            ...
            className={`w-auto object-fill transitions-theme ${
              ifExist
                ? "h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-60 rotate-6 hover:-rotate-12"
